...
bool CxImageJBG::Encode(CxFile * hFile)
{
	...
	if (head.biBitCount != 1){
		strcpy_s(info.szLastError,sizeof(info.szLastError),"JBG can save only 1-bpp images");
		return false;
	}
	...
	if (!buffer) {
		strcpy_s(info.szLastError,sizeof(info.szLastError),"Sorry, not enough memory available!");
		return false;
	}
	...
	if (hFile->Error()){
		strcpy_s(info.szLastError,sizeof(info.szLastError),"Problem while writing JBG file");
		return false;
	}
