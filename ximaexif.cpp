...
bool CxImageJPG::CxExifInfo::DecodeExif(CxFile * hFile, int32_t nReadMode)
{
    ...
    for(;;){
        ...
        if (SectionsRead >= MAX_SECTIONS){
			strcpy_s(m_szLastError,sizeof(m_szLastError),"Too many sections in jpg file");
			return false;
        }
        ...
        if (marker == 0xff){
            ...
            strcpy_s(m_szLastError,sizeof(m_szLastError),"too many padding bytes!");
			return false;
        }
        ...
        if (itemlen < 2){
			strcpy_s(m_szLastError,sizeof(m_szLastError),"invalid marker");
			return false;
        }
        ...
        if (Data == NULL){
            strcpy_s(m_szLastError,sizeof(m_szLastError),"Could not allocate memory");
			return false;
        }
        ...
        if (got != itemlen-2){
            strcpy_s(m_szLastError,sizeof(m_szLastError),"Premature end of file?");
			return false;
        }
        SectionsRead += 1;
        ...
        switch(marker){
            ...
            case M_SOS:   // stop before hitting compressed data 
                ...
                if (nReadMode & EXIF_READ_IMAGE){
                    ...
                    if (Data == NULL){
                        strcpy_s(m_szLastError,sizeof(m_szLastError),"could not allocate data for entire image");
						return false;
                    }

                    got = hFile->Read(Data, 1, size);
                    if (got != size){
                        strcpy_s(m_szLastError,sizeof(m_szLastError),"could not read the rest of the image");
						return false;
                    }
...
bool CxImageJPG::CxExifInfo::process_EXIF(uint8_t * CharBuf, uint32_t length)
{
    ...
    {   /* Check the EXIF header component */
        ...
        if (memcmp(CharBuf+0, ExifHeader,6)){
			strcpy_s(m_szLastError,sizeof(m_szLastError),"Incorrect Exif header");
			return false;
		}
    }
    ...
    if (memcmp(CharBuf+6,"II",2) == 0){
        ...
        if (memcmp(CharBuf+6,"MM",2) == 0){
            ...
            strcpy_s(m_szLastError,sizeof(m_szLastError),"Invalid Exif alignment marker.");
			return false;
        }
    }
    ...
    if (Get16u(CharBuf+8) != 0x2a){
        strcpy_s(m_szLastError,sizeof(m_szLastError),"Invalid Exif start (1)");
		return false;
    }
	...
	if (FirstOffset < 8 || FirstOffset > 16){
        ...
        strcpy_s(m_szLastError,sizeof(m_szLastError),"Suspicious offset of first IFD value");
		return false;
    }*/
                           ...
                           EXIFINFO * const m_exifinfo, uint8_t ** const LastExifRefdP, int32_t NestingLevel)
{
    ...
    if (NestingLevel > 4){
        strcpy_s(m_szLastError,sizeof(m_szLastError),"Maximum directory nesting exceeded (corrupt exif header)");
        return false;
    }
    ...
    if ((DirStart+2+NumDirEntries*12+2) > (OffsetBase+ExifLength)){
        strcpy_s(m_szLastError,sizeof(m_szLastError),"Illegally sized directory");
		return false;
    }
    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        if ((Format-1) >= NUM_FORMATS) {
            ...
            strcpy_s(m_szLastError,sizeof(m_szLastError),"Illegal format code in EXIF dir");
			return false;
		}
        ...
        if (ByteCount > 4){
            ...
            if (OffsetVal+ByteCount > ExifLength){
                ...
                strcpy_s(m_szLastError,sizeof(m_szLastError),"Illegal pointer offset value in EXIF.");
				return false;
            }
        ...
        switch(Tag){
            ...
            case TAG_MAKE:
				strncpy_s(m_exifinfo->CameraMake, sizeof(m_exifinfo->CameraMake), (char*)ValuePtr, 31);
                break;

            case TAG_MODEL:
				strncpy_s(m_exifinfo->CameraModel, sizeof(m_exifinfo->CameraModel), (char*)ValuePtr, 39);
                break;

			case TAG_EXIF_VERSION:
				strncpy_s(m_exifinfo->Version,sizeof(m_exifinfo->Version),(char*)ValuePtr, 4);
				break;

            case TAG_DATETIME_ORIGINAL:
				strncpy_s(m_exifinfo->DateTime, sizeof(m_exifinfo->DateTime), (char*)ValuePtr, 19);
                break;
            ...
            case TAG_USERCOMMENT:
                ...
                if (memcmp(ValuePtr, "ASCII",5) == 0){
                    for (a=5;a<10;a++){
                        ...
                        c = ((char*)ValuePtr)[a];
                        if (c != '\0' && c != ' '){
							strncpy_s(m_exifinfo->Comments, sizeof(m_exifinfo->Comments), (char*)ValuePtr+a, 199);
                            break;
                        }
                    }
                    
                }else{
					strncpy_s(m_exifinfo->Comments, sizeof(m_exifinfo->Comments), (char*)ValuePtr, 199);
                }
                break;
            ...
            case TAG_ORIENTATION:
                ...
                if (m_exifinfo->Orientation < 1 || m_exifinfo->Orientation > 8){
                    strcpy_s(m_szLastError,sizeof(m_szLastError),"Undefined rotation value");
        ...
        if (Tag == TAG_EXIF_OFFSET || Tag == TAG_INTEROP_OFFSET){
			...
			if (Offset>8){
				...
				if (SubdirStart < OffsetBase || 
					...
					strcpy_s(m_szLastError,sizeof(m_szLastError),"Illegal subdirectory link");
					return false;
				}
...
void CxImageJPG::CxExifInfo::process_COM (const uint8_t * Data, int32_t length)
{
    ...
    strcpy_s(m_exifinfo->Comments,sizeof(m_exifinfo->Comments),Comment);
}
////////////////////////////////////////////////////////////////////////////////
...
bool CxImageJPG::CxExifInfo::EncodeExif(CxFile * hFile)
{
    ...
    if (FindSection(M_SOS)==NULL){
		strcpy_s(m_szLastError,sizeof(m_szLastError),"Can't write exif : didn't read all");
		return false;
    }
