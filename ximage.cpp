...
void* CxImage::Create(uint32_t dwWidth, uint32_t dwHeight, uint32_t wBpp, uint32_t imagetype)
{
	...
	if ((dwWidth == 0) || (dwHeight == 0)){
		strcpy_s(info.szLastError,sizeof(info.szLastError),"CxImage::Create : width and height must be greater than zero");
		return NULL;
	}
	...
	{
		strcpy_s(info.szLastError,sizeof(info.szLastError),"CXIMAGE_MAX_MEMORY exceeded");
		return NULL;
	}
...
//    head.biClrImportant = 0;  See SetClrImportant
    ...
    if (!pDib){
		strcpy_s(info.szLastError,sizeof(info.szLastError),"CxImage::Create can't allocate memory");
		return NULL;
	}
