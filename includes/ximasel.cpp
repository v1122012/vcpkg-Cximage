...
bool CxImage::SelectionSet(CxImage &from)
{
	if (!from.IsGrayScale() || head.biWidth != from.head.biWidth || head.biHeight != from.head.biHeight){
		strcpy_s(info.szLastError,sizeof(info.szLastError),"CxImage::SelectionSet: wrong width or height, or image is not gray scale");
		return false;
	}
	...
	if (src==NULL || dst==NULL){
		strcpy_s(info.szLastError,sizeof(info.szLastError),"CxImage::SelectionSet: null pointer");
		return false;
	}
