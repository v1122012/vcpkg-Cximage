...
{
	if (hFile==NULL) {
		strcpy_s(info.szLastError,sizeof(info.szLastError),CXIMAGE_ERR_NOFILE);
		return true;
	}

	if (pDib==NULL){
		strcpy_s(info.szLastError,sizeof(info.szLastError),CXIMAGE_ERR_NOIMAGE);
		return true;
	}
	return false;
}
...
#ifdef WIN32
	if (_tfopen_s(&hFile,filename,_T("wb"))!=0)  return false;	// For UNICODE support
#else
...
{
	if (buffer!=NULL){
		strcpy_s(info.szLastError,sizeof(info.szLastError),"the buffer must be empty");
		return false;
	}
	CxMemFile file;
	file.Open();
...
bool CxImage::Encode(CxFile *hFile, uint32_t imagetype)
{
	...
	if (CXIMAGE_FORMAT_BMP==imagetype){
		...
		if (newima->Encode(hFile)){
			...
			strcpy_s(info.szLastError,sizeof(info.szLastError),newima->GetLastError());
			delete newima;
			return false;
		}
	}
	...
	strcpy_s(info.szLastError,sizeof(info.szLastError),"Encode: Unknown format");
	return false;
}
////////////////////////////////////////////////////////////////////////////////
/**
...
#endif
	strcpy_s(info.szLastError,sizeof(info.szLastError),"Multipage Encode, Unsupported operation for this format");
	return false;
}
...
//bool CxImage::Load(const char * filename, uint32_t imagetype)
{
...
#ifdef WIN32
		if (_tfopen_s(&hFile,filename,_T("rb"))!=0)  return false;	// For UNICODE support
#else
		if ((hFile=fopen(filename,"rb"))==NULL)  return false;
#endif
	...
	char szError[256];
	strcpy_s(szError,sizeof(szError),info.szLastError); //save the first error
	...
	if (!bOK && imagetype > 0) strcpy_s(info.szLastError,sizeof(info.szLastError),szError); //restore the first error
...
bool CxImage::LoadResource(HRSRC hRes, uint32_t imagetype, HMODULE hModule)
{
	...
	} else strcpy_s(info.szLastError,sizeof(info.szLastError),"Unable to load resource!");
	return false;
}
#endif //WIN32
////////////////////////////////////////////////////////////////////////////////
...
bool CxImage::Decode(CxFile *hFile, uint32_t imagetype)
{
	...
	strcpy_s(info.szLastError,sizeof(info.szLastError),"Decode: Unknown or wrong format");
	return false;
}
////////////////////////////////////////////////////////////////////////////////
/**
...
bool CxImage::CheckFormat(uint8_t * buffer, uint32_t size, uint32_t imagetype)
{
	if (buffer==NULL || size==NULL){
		strcpy_s(info.szLastError,sizeof(info.szLastError),"invalid or empty buffer");
		return false;
	}
	CxMemFile file(buffer,size);
