...
bool CxImage::Threshold(uint8_t level)
{
	...
	if (!tmp.IsValid()){
		strcpy_s(info.szLastError,sizeof(info.szLastError),tmp.GetLastError());
		return false;
	}
...
bool CxImage::Threshold(CxImage* pThresholdMask)
{
	...
	if (!pThresholdMask->IsValid() ||
		...
		strcpy_s(info.szLastError,sizeof(info.szLastError),"invalid ThresholdMask");
		return false;
	}
...
bool CxImage::GaussianBlur(float radius /*= 1.0f*/, CxImage* iDst /*= 0*/)
{
	...
	if (!tmp_x.IsValid()){
		strcpy_s(info.szLastError,sizeof(info.szLastError),tmp_x.GetLastError());
		return false;
	}
	...
	if (!tmp_y.IsValid()){
		strcpy_s(info.szLastError,sizeof(info.szLastError),tmp_y.GetLastError());
		return false;
	}
...
bool CxImage::SelectiveBlur(float radius, uint8_t threshold, CxImage* iDst)
{
	...
	if (!Dst.IsValid()){
		strcpy_s(info.szLastError,sizeof(info.szLastError),Dst.GetLastError());
		delete [] pPalette;
		return false;
	}
...
bool CxImage::Saturate(const int32_t saturation, const int32_t colorspace)
{
	...
	case 2:
		...
		break;
	default:
		strcpy_s(info.szLastError,sizeof(info.szLastError),"Saturate: wrong colorspace");
		return false;
	}
	return true;
}
	...
	for (int32_t y=0;y<h;y++){
		...
		for (int32_t x=0;x<w;x++){
			...
			case 4:
				...
				break;
			default:
				strcpy_s(info.szLastError,sizeof(info.szLastError),"ConvertColorSpace: unknown source colorspace");
				return false;
			}
			...
			case 4:
				...
				break;
			default:
				strcpy_s(info.szLastError,sizeof(info.szLastError),"ConvertColorSpace: unknown destination colorspace");
				return false;
			}
			BlindSetPixelColor(x,y,c);
		}
...
int32_t  CxImage::OptimalThreshold(int32_t method, RECT * pBox, CxImage* pContrastMask)
{
	...
	if (head.biBitCount!=8){
		strcpy_s(info.szLastError,sizeof(info.szLastError),"OptimalThreshold works only on 8 bit images");
		return -1;
	}
	...
	if (pContrastMask){
		if (!pContrastMask->IsValid() ||
			...
			strcpy_s(info.szLastError,sizeof(info.szLastError),"OptimalThreshold invalid ContrastMask");
			return -1;
		}
	}
...
bool CxImage::AdaptiveThreshold(int32_t method, int32_t nBoxSize, CxImage* pContrastMask, int32_t nBias, float fGlobalLocalBalance)
{
	...
	if (pContrastMask){
		if (!pContrastMask->IsValid() ||
			...
			pContrastMask->GetWidth() != GetWidth() ||
			pContrastMask->GetHeight() != GetHeight()){
			strcpy_s(info.szLastError,sizeof(info.szLastError),"AdaptiveThreshold invalid ContrastMask");
			return false;
		}
	}
