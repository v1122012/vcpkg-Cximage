...
bool CxImageJPG::Encode(CxFile * hFile)
{
	...
	if (head.biClrUsed!=0 && !IsGrayScale()){
		strcpy_s(info.szLastError,sizeof(info.szLastError),"JPEG can save only RGB or GreyScale images");
		return false;
	}	
	...
	*/
	...
	if (setjmp(jerr.setjmp_buffer)) {
		...
		* We need to clean up the JPEG object, close the input file, and return.
		*/
		strcpy_s(info.szLastError,sizeof(info.szLastError), jerr.buffer); //<CSC>
