...
/////////////////////////////////////////////////////////////////////
		...
		if (!hMeta){
			strcpy_s(info.szLastError,sizeof(info.szLastError),"corrupted WMF");
	...
	if (info.nEscape == -1) {	// Check if cancelled
		...
		DeleteEnhMetaFile(hMeta);
		strcpy_s(info.szLastError,sizeof(info.szLastError),"output dimensions returned");
		return true;
	}

	if (!cx || !cy)	{
		DeleteEnhMetaFile(hMeta);
		strcpy_s(info.szLastError,sizeof(info.szLastError),"empty WMF");
		return false;
	}
		...
		if (hBitmap){
			...
			if (iEntries) { 
				if ((plogPal = (PLOGPALETTE)GlobalAlloc(GMEM_FIXED | GMEM_ZEROINIT, 
					...
					DeleteDC(hDC);
					DeleteEnhMetaFile(hMeta);
					strcpy_s(info.szLastError,sizeof(info.szLastError),"Cancelled");
					return false;
				} 
...
{
	if (hFile == NULL) return false;
	strcpy_s(info.szLastError,sizeof(info.szLastError), "Save WMF not supported");
	return false;
}
