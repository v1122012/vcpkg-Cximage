	...
	if (hbmp) { 
		...
		else
		{
			...
			if (GetDIBits(dc, hbmp, 0, head.biHeight, info.pImage,
				...
				strcpy_s(info.szLastError,sizeof(info.szLastError),"GetDIBits failed");
				::ReleaseDC(NULL, dc);
				return false;
			}
			::ReleaseDC(NULL, dc);
...
//int32_t CxImage::DrawString(HDC hdc, int32_t x, int32_t y, const char* text, RGBQUAD color, const char* font, int32_t lSize, int32_t lWeight, uint8_t bItalic, uint8_t bUnderline, bool bSetAlpha)
{
	if (IsValid()){
		...
		_tcsncpy_s(m_pLF->lfFaceName,sizeof(m_pLF->lfFaceName)/sizeof(m_pLF->lfFaceName[0]),font,31);	// For UNICODE support
...
void CxImage::InitTextInfo( CXTEXTINFO *txt )
{
	...
	_tcscpy_s( txt->lfont.lfFaceName,sizeof(txt->lfont.lfFaceName)/sizeof(txt->lfont.lfFaceName[0]), _T("Arial")); //use TCHAR mappings <Cesar M>
    ...
    // the text 
	_tcscpy_s( txt->text, sizeof(txt->text)/sizeof(txt->text[0]), _T("Sample Text")); // text use TCHAR mappings <Cesar M>
    txt->align = DT_CENTER;
    return;
}
