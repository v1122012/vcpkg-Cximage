...
bool CxImageJAS::Decode(CxFile *hFile, uint32_t imagetype)
{
  ...
  cx_try
  {
	...
	char szfmt[4];
	*szfmt = '\0';
	strncpy_s(szfmt,sizeof(szfmt),jas_image_fmttostr(fmt),3);
	szfmt[3] = '\0';
	...
	if (strcmp(message,"")) strncpy_s(info.szLastError,sizeof(info.szLastError),message,255);
...
bool CxImageJAS::Encode(CxFile * hFile, uint32_t imagetype)
{
	...
	if (head.biClrUsed!=0 && !IsGrayScale()){
		strcpy_s(info.szLastError,sizeof(info.szLastError),"JasPer can save only RGB or GrayScale images");
		return false;
	}
  ...
  cx_try {
	...
	*szfmt = '\0';
#if CXIMAGE_SUPPORT_JP2
	if (imagetype == CXIMAGE_FORMAT_JP2) strcpy_s(szfmt,sizeof(szfmt),"jp2");
#endif
#if CXIMAGE_SUPPORT_JPC
	if (imagetype == CXIMAGE_FORMAT_JPC) strcpy_s(szfmt,sizeof(szfmt),"jpc");
#endif
#if CXIMAGE_SUPPORT_RAS
	if (imagetype == CXIMAGE_FORMAT_RAS) strcpy_s(szfmt,sizeof(szfmt),"ras");
#endif
#if CXIMAGE_SUPPORT_PNM
	if (imagetype == CXIMAGE_FORMAT_PNM) strcpy_s(szfmt,sizeof(szfmt),"pnm");
#endif
#if CXIMAGE_SUPPORT_PGX
	if (imagetype == CXIMAGE_FORMAT_PGX){
		strcpy_s(szfmt,sizeof(szfmt),"pgx");
	...
	char szoutopts[32];
	sprintf_s(szoutopts,sizeof(szoutopts),"rate=%.3f", info.fQuality/100.0f);
